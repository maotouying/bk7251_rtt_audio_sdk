...
static void record_thread_entry(void *parameter)
{
    ...
    /* initialize mempool */
    recorder->mp_block_size = recorder->sample_rate / 50 * 2;
    recorder->mp_cnt = 10;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            read_bytes = rt_sound_read(device, 0, buffer, recorder->mp_block_size);
            /*encode ....*/
            record_msg_send(recorder, buffer, RECORD_MSG_DATA, read_bytes);
        }
...
static int record_pcm_tcp(int argc, char **argv)
{
